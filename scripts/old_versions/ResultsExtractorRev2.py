...
def dbase_extract(run_num):
    ...
    SQL = 'SELECT Options.OPT_DESC,Options.IRIAV,OptionEconomics.NPV,OptionEconomics.IRR FROM Options INNER JOIN OptionEconomics ON OptionEconomics.OPT_ID = Options.OPT_ID'
    rows = cur.execute(SQL).fetchall()
    SQL = 'SELECT HDM4RunData.DESC FROM HDM4RunData';
    run = cur.execute(SQL).fetchall()
    # SQL = 'SELECT Options.IRIAV FROM Options';
    # iri = cur.execute(SQL).fetchall()
    cur.close()
    con.close()
    named_rows = []
    ...
    if has_top_row == False:
        with open(opfile, 'wb') as output:
            ...
            toprow = ['Run Title','Alternative','AVE_IRI','NPV','IRR']
